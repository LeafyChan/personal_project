...
2. **Review Modal not loading the PDF — partially fixed; fully fixed in Session 10.** The backend was already correctly returning a Drive-fallback `drive_file_id`; `ReviewModal.jsx` was just never reading it. Fixed to embed it as an iframe `src`. The fallback URL pattern (`/view`) was itself still wrong at the end of Session 9 — corrected to `/preview` in Session 10, see below.
...
### ✅ Resolved (Session 10 — extraction prompt fix, reconciliation validation, Review Modal rebuild)

4. **Vendor/buyer name extraction swallowing the entire seller block — FIXED.** Root cause confirmed in `core/extractor.py`'s `EXTRACTION_INSTRUCTIONS_TEMPLATE`: the prompt told the model `vendor_name`/`buyer_name` exist as fields, but never said where the name ends and the address/Tax-Id/GSTIN block begins. With no boundary instruction, the model captured the entire contiguous text block under the "Seller:"/"Client:" heading rather than just the company name on its own line. Fixed by adding explicit boundary rules to `GENERAL RULES` with a worked example using the exact TechVision/Jaipur invoice that surfaced the bug, plus an explicit two-column-layout rule ("never let text from one column bleed into the other's name field") and a rule to still capture a non-15-character "Tax Id" value as `vendor_gstin`/`buyer_gstin` rather than discarding it or folding it into the name just because it doesn't match the Indian GSTIN format.
5. **No amount-reconciliation validation on `PATCH /invoices/{id}` — FIXED.** Added `_reconcile_invoice_amounts()` in `main.py`, called after all field/line-item updates in a request are applied, re-reading the final state fresh from the DB (never trusting a stale in-memory value from earlier in the chain of edits). Checks `sum(line_items.amount)` vs `taxable_amount`, and `taxable_amount + total_gst_amount` vs `total_amount`, reusing the SAME `amount_reconciliation_tolerance` value `validator.py` already uses at OCR time (loaded via the existing `core_pipeline.load_schema(SCHEMA_PATH)` call already used elsewhere in `main.py`) rather than hardcoding a second, possibly-drifting tolerance. Per the project decision: **never rejects the save** — applies it, but flags `status = 'WARNING'` with the specific mismatch text, and clears that flag automatically only when a later edit makes the same specific check pass again (and only when the existing warning text actually matches this check's own phrasing, never a `WARNING` set for an unrelated reason like a GSTIN format problem — see the worked trace in Session 10 below for why this distinction matters). Verified by hand-tracing the exact 100+200→edit total to 310→edit line item to 150 scenario from the request; the WARNING correctly persists across both edits rather than getting incorrectly cleared partway through.
6. **Review Modal missing line items entirely — FIXED.** Added a full line-items section to `ReviewModal.jsx`: description, HSN/SAC, quantity, rate, amount, and the previously-completely-absent per-line tax % (`line_tax_rate_percent`), all editable, saving through the same partial-PATCH contract `InvoiceList.jsx`'s line-item editor already uses (`COALESCE`-based on the backend, so omitting a field keeps its current value). Verified end-to-end with an actual render test: edited a line item's rate from 100→150, confirmed the captured PATCH body was exactly `{"line_items": [{"line_item_id": "li-1", "rate": 150}]}` — no extra fields, no stale values sent.
7. **Review Modal's PDF viewer popping a new tab and prompting a Google sign-in — FIXED.** Confirmed the root cause: the Drive-fallback iframe was using `https://drive.google.com/file/d/{id}/view` — Drive's full interactive viewer, which is auth-gated and can refuse to render inside an iframe at all (exactly the reported symptom). Changed to `https://drive.google.com/file/d/{id}/preview`, the embeddable variant with no sign-in prompt and no new-tab behavior. Verified via render test that the iframe's `src` attribute resolves to the correct `/preview` URL.

Also fully removed the still-recurring duplicate `shouldAutoReview` export from `ReviewModal.jsx` (third time this exact duplication has crept back in across sessions — it belongs only in `InvoiceReviewUtils.js`). **Worth double-checking on your end why this keeps reappearing** — it suggests edits are sometimes being made against an older saved copy of `ReviewModal.jsx` rather than the most recently fixed one; see the note in Session 10 below.
...
### Design gap — decided AND implemented in Session 10

4. **No reconciliation check between line items and header totals; editing one doesn't validate against, or recompute, the other. — IMPLEMENTED.**
   ...
   **Decision:** on a save that doesn't reconcile, **do not reject it** — apply the edit, flag `status = 'WARNING'`, surface the specific mismatch.
   **Status: implemented and verified** — see `_reconcile_invoice_amounts()` in `main.py` and the Session 10 write-up below for the full hand-traced verification of the exact scenario above.
